...
class HEDUNet(nn.Module):
    ...
    def __init__(self, input_channels, side_channels, output_channels=2, base_channels=16,
    ...
    def inspect_forward(self, x, side):
        B, _, H, W = x.shape
        x = self.init(x)
        ...
        if self.merging == 'attention':
            queries_multires = [q(feat) for q, feat in zip(self.queries, multilevel_features)]
            queries = [F.interpolate(q, size=(H, W), mode='bilinear', align_corners=True) for q in queries_multires]
            queries = torch.cat(queries, dim=1)
            queries = queries.reshape(B, -1, self.output_channels, H, W)
            attn = F.softmax(queries, dim=1)
            predictions = predictions.reshape(B, -1, self.output_channels, H, W)
